...
.DESCRIPTION
    ...
    It supports multiple operation modes including privileged access and general access. The script can operate exclusively on Windows or Linux assets, or both simultaneously, with additional features like SSH key usage for Linux and credential management for Windows.

.PARAMETER Windows
    Indicates that the script will operate on Windows assets. This switch is mandatory when operating in the 'Windows' parameter set.

.PARAMETER Linux
    Indicates that the script will operate on Linux assets. This switch is mandatory when operating in the 'Linux' parameter set.

.PARAMETER WindowsAndLinux
    Indicates that the script will operate on both Windows and Linux assets simultaneously.

.PARAMETER LinuxUseSSHKey
    When operating on Linux assets, this switch enables the use of an SSH key for authentication.

.PARAMETER LinuxSSHKeyPath
    Path to the SSH private key file used for Linux authentication. Use with -LinuxUseSSHKey. When supplied, the interactive prompt is skipped and no deprecation warning is emitted.

.PARAMETER Mode
    Specifies the operation mode of the script. Valid options are 'Privileged' or 'All'. This parameter is applicable to both Windows and Linux.
...
.PARAMETER IPAddress
    Allows running breakglass on a single asset identified by IP address.
...
.EXAMPLE
    PS> .\BreakGlass.ps1 -Linux -LinuxUseSSHKey -LinuxSSHKeyPath C:\keys\id_rsa -Network -Mode All -ServersOnly
    Runs the script for Linux server assets, opening all inbound ports using an SSH key.

.EXAMPLE
    PS> .\BreakGlass.ps1 -Asset linuxserver.contoso.local -Linux -LinuxUseSSHKey -LinuxSSHKeyPath C:\keys\id_rsa -Network -Mode All
...
.EXAMPLE
    ...
    Runs the script for a single windows asset, opening all inbound ports requesting credentials from the user interactively.
...
Function Add-DomainCredential {
    ...
    if ($Password) {
        ...
        $credential = Get-Credential -Message $PasswordPrompt -UserName $userName
    }
...
if ($PSBoundParameters['Linux'] -or $PSBoundParameters['WindowsAndLinux']) {
    ...
    if (!$PSBoundParameters['LinuxUseSSHKey']) {
        ...
        if ($PSBoundParameters['LinuxSSHKeyPath']) {
            $global:linuxSSHKey = $LinuxSSHKeyPath
        } else {
