function Sync-ADUserStatus {
    ...
    .DESCRIPTION
        ...
        inactive via the ZN API. The ZN audit comment carries the AD whenChanged timestamp — the
        closest proxy for a disable date that AD natively provides — plus an optional admin note.

        Matching uses SID as the primary key, falling back to UserPrincipalName.
    ...
    .PARAMETER Note
        Admin note appended to each ZN audit comment (e.g., "Q2 offboarding").
    .PARAMETER PassThru
        Emit one result object per user to the pipeline after the sync completes.
    .EXAMPLE
        Sync-ZNADUserStatus -WhatIf
        Preview which users would be inactivated without making any changes.
    .EXAMPLE
        Sync-ZNADUserStatus -Domain corp.example.com -Note "Q2 offboarding"
        Inactivate disabled AD users in ZN, tagging each audit entry with a note.
    .EXAMPLE
        Sync-ZNADUserStatus -PassThru | Where-Object Action -eq 'Inactivated' | Export-Csv offboarded.csv -NoTypeInformation
        Run the sync and pipe inactivated users to a CSV report.
    .AUTHOR
        Olaf Gradin
    #>
    ...
    # source=3 = Active Directory; operation=1 = equals
    $sourceFilter = [Uri]::EscapeDataString('[{"field":"source","operand":3,"operation":1}]')
    $pageSize     = 100
    ...
    if (-not $isWhatIf -and $toProcess.Count -gt 0) {
        ...
        if ($PSCmdlet.ShouldProcess($target, 'Inactivate in Zero Networks')) {
            foreach ($entry in $toProcess) {
                ...
                if ($entry.AdUser.whenChanged) {
                    $commentParts += "AD whenChanged: $($entry.AdUser.whenChanged.ToString('yyyy-MM-dd'))"
                }
                if ($entry.ZNDom) { $commentParts += "Domain: $($entry.ZNDom)" }
                if ($Note)        { $commentParts += "Note: $Note"              }

                $body = @{ comment = ($commentParts -join ' | ') } | ConvertTo-Json -Compress

                try {
                    Invoke-RestMethod `
                        -Uri     "$baseUrl/users/$($entry.ZNId)/actions/inactivate" `
                        -Method  Post `
                        -Headers $headers `
                        -Body    $body | Out-Null
                    ...
                    Write-Warning "Failed to inactivate $($entry.ZNName): Status $code"
    ...
    if ($isWhatIf) { Write-Host '  Run without -WhatIf to apply.' -ForegroundColor DarkGray }
    Write-Host ''
